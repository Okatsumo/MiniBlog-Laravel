    ...
    static remove(userId){
        return new Promise((resolve, reject) => {
            api.call('delete', '/api/user/' + userId)
                .then(() => {
                    Vue.notify({group: 'auth', title: 'Удаление пользователя', text: 'Пользователь был успешно удален'});
                })
                .finally(() => {
                    resolve(true);
                })
                .catch(({response}) => {
                    if (response.status !== 200) {
                        Vue.notify({group: 'auth', title: 'Удаление пользователя', text: 'Произошла ошибка'});
                    }
                    reject(response)
                });
        })
